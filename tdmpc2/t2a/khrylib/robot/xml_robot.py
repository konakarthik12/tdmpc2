...
class Body:
    ...
    def parse_param_specs(self):
        ...
        # @sanghyun: change DictConfig to dict
        self.param_specs = OmegaConf.to_object(self.param_specs)
        for name, specs in self.param_specs.items():
            # @sanghyun: not only list, "OmegaConf" object is also considered as list
            # because tdmpc2 uses hydra...
            if 'lb' in specs:
                lb_list = specs['lb']
                if isinstance(lb_list, ListConfig):
                    lb_list = lb_list._content
                if isinstance(lb_list, list):
                    specs['lb'] = np.array(lb_list)
            
            if 'ub' in specs:
                ub_list = specs['ub']
                if isinstance(ub_list, ListConfig):
                    ub_list = ub_list._content
                if isinstance(ub_list, list):
                    specs['ub'] = np.array(ub_list)
